...
class Fragment:
    ...
    feature: str
    feature_name: str
    kind: str  # overview | requirement | rule | bug | finding
...
def reindex(
    prd_dir: Path, index_path: Path | None = None, *, embeddings: bool = True
) -> dict[str, Any]:
...
def _status_dict(snap: _Snapshot | None) -> dict[str, Any]:
    if snap is None:
        return {"exists": False, "indexed_at": None, "fragment_count": 0, "has_embeddings": False}
    return {
        "exists": True,
