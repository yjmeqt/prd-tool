...
- `search.py` — hybrid search index. `iter_fragments()` walks every PRD into searchable fragments (overview / requirement / rule / bug / finding / **image**) each carrying the DOM anchor the viewer scrolls to (`R<n>`, `R<n>.<ruleid>`, `bug.<id>`; images anchor to their containing rule/requirement). `reindex()` builds a snapshot — CJK-aware BM25 tokens + local fastembed sentence vectors (`paraphrase-multilingual-MiniLM-L12-v2`) — and writes it to a single repo-level sidecar (`<repo_root>/.prd-tool-search.json`, kept **outside** `prd_dir` so the file-watcher ignores it; git-ignored). `search()` loads the cached snapshot (invalidated by mtime), runs BM25 + cosine over the **same** fragment set, and fuses the two rankings with RRF. fastembed/onnxruntime are lazy-imported; if embeddings are unavailable the index degrades to lexical-only (`has_embeddings: false`). Indexing is manual (`prd index` / the viewer's Reindex button), never automatic.
  - **Image content** (`prd index --clip`; OCR needs the optional `[vision]` extra): each local `<img>` becomes a `kind="image"` fragment enriched by two best-effort, lazy-imported stages, cached per file content hash (`.prd-tool-ocr-cache.json`): **OCR** (RapidOCR) text folded into the searchable index, and **CLIP** (`clip-ViT-B-32`, via fastembed) image vectors in a *separate* space — `search()` runs a CLIP text→image pass and returns a `visual` result group alongside the text `results`. Both degrade to empty if a model/dep is missing. (A local-VLM component-tagging stage was prototyped and removed: small VLMs tagged unreliably and added little over OCR + CLIP + the indexed `figma_node` names.)
